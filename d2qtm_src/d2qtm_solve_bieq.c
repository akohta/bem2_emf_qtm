  ...
  for(t=1;t<=Ne;t++){
    ...
    for(j=0;j<3;j++){
      ...
      for(s=1;s<=Ne;s++){
        ...
        if(md->PD==1 && did==0){
          if(t!=s)   coefficient_qpgf_NV(CC,rt,sd,md);
          else       coefficient_qpgf_bd_eta(CC,md->bd.et[j],sd,md);
        }
        else {
...
void calc_bv_vw(DOMD *md)
{
  double complex FD_1(int order,double h,double x0,int did,int s,DOMD *md); // d2qtm_field.c
  ...
  for(d=0;d<=md->MN;d++){
    ...
    for(s=1;s<=md->bd.sb[d].Ne;s++){
      ...
      for(i=0;i<3;i++){
        ...
        //dudt=dHzdt_bv_node_ndmtd(d,s,i,md);
        dudt=i_aJ*FD_1(FDO,FDH,md->bd.et[i],d,s,md);
...
void create_amatrix_subdomain(int did,CMD *cm,DOMD *md)
{
  ...
  for(j=0;j<cm->N;j++){
    if(fread(tG,sizeof(double complex),cm->N,fg)!=cm->N){
      printf("d2qtm_solve_bieq.c, create_amatrix_subdomain(), failed to read the tG. exit...\n");
      exit(1);
    }
    for(s=0;s<cm->N;s++){
...
void create_bmatrix_subdomain(int did,CMD *cm,double complex **u)
{
  ...
  for(j=0;j<cm->N;j++){
    ...
    if(fread(tH,sizeof(double complex),cm->N,fh)!=cm->N){
      printf("d2qtm_solve_bieq.c, create_bmatrix_subdomain(), failed to read the tH. exit...\n");
      exit(1);
    }
    for(s=1;s<=cm->N/3;s++){
      for(sn=0;sn<3;sn++){
