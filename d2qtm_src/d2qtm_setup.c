...
void dat_read (char *fname,DOMD *md)
{
  ...
  if(fread(md->med_fn,sizeof(char),128,fp)!=128){
    printf("d2qtm_setup.c, dat_read(), failed to read the med_fn. exit...\n");
    exit(1);
  }
  if(fread(md->msh_fn,sizeof(char),128,fp)!=128){
    printf("d2qtm_setup.c, dat_read(), failed to read the msh_fn. exit...\n");
    exit(1);
  }
  if(fread(md->prd_fn,sizeof(char),128,fp)!=128){
    printf("d2qtm_setup.c, dat_read(), failed to read the prd_fn. exit...\n");
    exit(1);
  }
  // rotation and translation data
  if(fread(&(md->ra),sizeof(double),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the ra. exit...\n");
    exit(1);
  }
  if(fread(&(md->tx),sizeof(double),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the tx. exit...\n");
    exit(1);
  }
  if(fread(&(md->ty),sizeof(double),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the ty. exit...\n");
    exit(1);
  }
  // material def
  if(fread(&(md->MN),sizeof(int),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the MN. exit...\n");
    exit(1);
  }
  ...
  if(fread(md->n,sizeof(double complex),md->MN+1,fp)!=md->MN+1){
    printf("d2qtm_setup.c, dat_read(), failed to read the n. exit...\n");
    exit(1);
  }
  if(fread(&(md->PD),sizeof(int),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the PD. exit...\n");
    exit(1);
  }
  // incident field def
  if(fread(&(md->wd),sizeof(INFD),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the wd. exit...\n");
    exit(1);
  }
  // hm_qgf def
  if(fread(&(md->qd),sizeof(QPDT),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the qd. exit...\n");
    exit(1);
  }
  // boundary def
  if(fread(&(md->bd.Nn),sizeof(int),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the Nn. exit...\n");
    exit(1);
  }
  if(fread(&(md->bd.Ne),sizeof(int),1,fp)!=1){
    printf("d2qtm_setup.c, dat_read(), failed to read the Ne. exit...\n");
    exit(1);
  }
  malloc_node(&(md->bd));  malloc_elem(&(md->bd)); // malloc
  if(fread(md->bd.et,sizeof(double),7,fp)!=7){
    printf("d2qtm_setup.c, dat_read(), failed to read the et. exit...\n");
    exit(1);
  }
  if(fread(md->bd.wg,sizeof(double),7,fp)!=7){
    printf("d2qtm_setup.c, dat_read(), failed to read the wg. exit...\n");
    exit(1);
  }
  if(fread(md->bd.wk,sizeof(double),7,fp)!=7){
    printf("d2qtm_setup.c, dat_read(), failed to read the wk. exit...\n");
    exit(1);
  }
  if(fread(md->bd.M,sizeof(double),21,fp)!=21){
    printf("d2qtm_setup.c, dat_read(), failed to read the M. exit...\n");
    exit(1);
  }
  if(fread(md->bd.xn,sizeof(double),md->bd.Nn+1,fp)!=md->bd.Nn+1){
    printf("d2qtm_setup.c, dat_read(), failed to read the xn. exit...\n");
    exit(1);
  }  
  if(fread(md->bd.yn,sizeof(double),md->bd.Nn+1,fp)!=md->bd.Nn+1){
    printf("d2qtm_setup.c, dat_read(), failed to read the yn. exit...\n");
    exit(1);
  }
  if(fread(md->bd.md, sizeof(int),md->bd.Ne+1,fp)!=md->bd.Ne+1){
    printf("d2qtm_setup.c, dat_read(), failed to read the md. exit...\n");
    exit(1);
  }  
  if(fread(md->bd.sd, sizeof(int),md->bd.Ne+1,fp)!=md->bd.Ne+1){
    printf("d2qtm_setup.c, dat_read(), failed to read the sd. exit...\n");
    exit(1);
  }
  if(fread(md->bd.egd,sizeof(int),md->bd.Ne+1,fp)!=md->bd.Ne+1){
    printf("d2qtm_setup.c, dat_read(), failed to read the egd. exit...\n");
    exit(1);
  }
  if(fread(md->bd.xh,sizeof(double),GLH,fp)!=GLH){
    printf("d2qtm_setup.c, dat_read(), failed to read the xh. exit...\n");
    exit(1);
  }  
  if(fread(md->bd.wh,sizeof(double),GLH,fp)!=GLH){
    printf("d2qtm_setup.c, dat_read(), failed to read the wh. exit...\n");
    exit(1);
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.ed[i],sizeof(int),3,fp)!=3){
      printf("d2qtm_setup.c, dat_read(), failed to read the ed[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.x[i],sizeof(double),7,fp)!=7){
      printf("d2qtm_setup.c, dat_read(), failed to read the x[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.y[i],sizeof(double),7,fp)!=7){
      printf("d2qtm_setup.c, dat_read(), failed to read the y[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.dx[i],sizeof(double),7,fp)!=7){
      printf("d2qtm_setup.c, dat_read(), failed to read the dx[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.dy[i],sizeof(double),7,fp)!=7){
      printf("d2qtm_setup.c, dat_read(), failed to read the dy[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.ui[i],sizeof(double complex),3,fp)!=3){
      printf("d2qtm_setup.c, dat_read(), failed to read the ui[i]. exit...\n");
      exit(1);
    }
  }
  for(i=0;i<=md->bd.Ne;i++){
    if(fread(md->bd.duidn[i],sizeof(double complex),3,fp)!=3){
      printf("d2qtm_setup.c, dat_read(), failed to read the duidn[i]. exit...\n");
      exit(1);
    }
  }
  // sub domain def
  ...
  for(i=0;i<=md->MN;i++){
    if(fread(md->bd.sb[i].eid,sizeof(int),md->bd.sb[i].Ne+1,fp)!=md->bd.sb[i].Ne+1){
      printf("d2qtm_setup.c, dat_read(), failed to read the eid. exit...\n");
      exit(1);
    }
    for(j=0;j<=md->bd.sb[i].Ne;j++){
      if(fread(md->bd.sb[i].u[j],sizeof(double complex),3,fp)!=3){
        printf("d2qtm_setup.c, dat_read(), failed to read the u[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].v[j],sizeof(double complex),3,fp)!=3){
        printf("d2qtm_setup.c, dat_read(), failed to read the v[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].w[j],sizeof(double complex),3,fp)!=3){
        printf("d2qtm_setup.c, dat_read(), failed to read the w[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].dudn[j],sizeof(double complex),3,fp)!=3){
        printf("d2qtm_setup.c, dat_read(), failed to read the dudn[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].dvdn[j],sizeof(double complex),3,fp)!=3){
        printf("d2qtm_setup.c, dat_read(), failed to read the dvdn[j]. exit...\n");
        exit(1);
      }
      if(fread(md->bd.sb[i].dwdn[j],sizeof(double complex),3,fp)!=3){
        printf("d2qtm_setup.c, dat_read(), failed to read the dwdn[j]. exit...\n");
        exit(1);
      }
    }
  }
...
void output_node_particles(char *fname,DOMD *md)
{
  ...
  s1=strlen(fname);
  if(s1>200){
    printf("d2qtm_setup.c, output_node_particles(), file name is too long. exit...\n");
    exit(1);
  }
  sprintf(fo,"%s",fname);
  sd=strrchr(fo,'.');
...
void read_medium_data(char *med_fn,DOMD *md)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("d2qtm_setup.c, read_medium_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fgets(buf,256,fp)==NULL){
    printf("d2qtm_setup.c, read_medium_data(), failed to read the line. exit...\n");
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("d2qtm_setup.c, read_medium_data(), failed to read the MN. exit...\n");
    exit(1);  
  }
  md->MN=ti;
  ...
  for(i=1;i<=md->MN;i++){
    if(fscanf(fp,"%lf",&td)!=1){
      printf("d2qtm_setup.c, read_medium_data(), failed to read the real(n). exit...\n");
      exit(1);
    }
    if(fscanf(fp,"%lf",&td2)!=1){
      printf("d2qtm_setup.c, read_medium_data(), failed to read the imag(n). exit...\n");
      exit(1); 
    }
    md->n[i]=td+td2*I;
  }
...
void read_mesh_data(char *msh_fn,DOMD *md)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("d2qtm_setup.c, read_mesh_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fscanf(fp,"%lf",&td)!=1){
    printf("d2qtm_setup.c, read_mesh_data(), failed to read the file version. exit...\n");
    exit(1);
  }
  // check file version
  if(td<MSHVER){
  ...
  if(fscanf(fp,"%d",&ti)!=1){
    printf("d2qtm_setup.c, read_mesh_data(), failed to read the data format. exit...\n");
    exit(1);
  }
  //check data format
  if(ti!=MSHASCI){
  ...
  if(fscanf(fp,"%d\n",&ti)!=1){
    printf("d2qtm_setup.c, read_mesh_data(), failed to read the precision. exit...\n");
    exit(1);
  }
  ...
  if(fscanf(fp,"%d",&ti)!=1){
    printf("d2qtm_setup.c, read_mesh_data(), failed to read the Nn. exit...\n");
    exit(1);
  }
  md->bd.Nn=ti;
  malloc_node(&(md->bd));
  for(i=1;i<=md->bd.Nn;i++){
    if(fscanf(fp,"%d",&ti)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the id. exit...\n");
      exit(1);
    }
    if(ti!=i) printf("bad id %d\n",ti);
    if(fscanf(fp,"%lf",&td)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the node x. exit...\n");
      exit(1);
    }
    md->bd.xn[i]=td;
    if(fscanf(fp,"%lf",&td)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the node y. exit...\n");
      exit(1);
    }
    md->bd.yn[i]=td;
    if(fscanf(fp,"%lf\n",&td)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the node z. exit...\n");
      exit(1);
    }
  }
  if(fgets(buf,256,fp)==NULL){
  ...
  if(fscanf(fp,"%d",&ti)!=1){
    printf("d2qtm_setup.c, read_mesh_data(), failed to read the Ne. exit...\n");
    exit(1);
  }
  md->bd.Ne=ti/2;
  malloc_elem(&(md->bd));
  for(i=1;i<=md->bd.Ne;i++){
    // main
    if(fscanf(fp,"%d",&ti)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the main id. exit...\n");
      exit(1);
    }
    if(ti!=i*2-1)    printf("bad id %d\n",ti);
    if(fscanf(fp,"%d",&ti)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the element type. exit...\n");
      exit(1);
    }
    if(ti!=ELEMTYPE) printf("bad element type. element type must be %d\n",ELEMTYPE);
    if(fscanf(fp,"%d",&ti)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the ti. exit...\n");
      exit(1);
    }
    for(j=0;j<ti;j++){
      if(fscanf(fp,"%d",&ti2)!=1){
        printf("d2qtm_setup.c, read_mesh_data(), failed to read the ti2. exit...\n");
        exit(1);
      }
      if(j==0){
    ...
    if(fscanf(fp,"%d",&ti)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the ed0. exit...\n");
      exit(1);
    }
    md->bd.ed[i][0]=ti;
    if(fscanf(fp,"%d",&ti)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the ed1. exit...\n");
      exit(1);
    }
    md->bd.ed[i][1]=ti;
    if(fscanf(fp,"%d",&ti)!=1){
    ...
    if(fscanf(fp,"%d",&ti)!=1){
      printf("d2qtm_setup.c, read_mesh_data(), failed to read the ed2. exit...\n");
      exit(1);
    }
...
void read_periodicity_data(char *prd_fn,DOMD *md)
{
  ...
  if(fgets(buf,256,fp)==NULL){
    printf("d2qtm_setup.c, read_periodicity_data(), failed to read the line. exit...\n");
    exit(1);
  }
  if(fgets(buf,256,fp)==NULL){
    printf("d2qtm_setup.c, read_periodicity_data(), failed to read the line. exit...\n");
  ...
  if(fscanf(fp,"%d %lf",&ti,&td)!=2){
    printf("d2qtm_setup.c, read_periodicity_data(), failed to read the PD and d. exit...\n");
    exit(1);
  } 
  md->PD=ti;
  md->qd.d=td;
