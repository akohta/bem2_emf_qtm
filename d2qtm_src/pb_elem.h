...
typedef struct domain_data{
  char med_fn[128],msh_fn[128],prd_fn[128]; // medium_datafile_name , mesh data file name, periodicity datafile name
  ...
  int PD; // periodicity setting, PD=1 : quasi-periodic boundary condition, PD=0 : no periodicity
...
// -- pb_coef_qpgf.c --
void coefficient_qpgf_GL(double complex *CC,double *rt,int s,DOMD *md);
...
int  coefficient_qpgf_NV(double complex *CC,double *rt,int s,DOMD *md);
